...
deploy-operator-package: push-operator-image get-tag
	...
	@sed -i -e 's,REPLACE_IMAGE,$(POSTGRESQL_OPERATOR_IMAGE):$(POSTGRESQL_OPERATOR_VERSION)-$(TAG),' $(OPERATOR_MANIFESTS)/postgresql-operator.v$(POSTGRESQL_OPERATOR_VERSION).clusterserviceversion-v$(POSTGRESQL_OPERATOR_VERSION).yaml
	operator-courier --verbose verify $(OPERATOR_MANIFESTS)
...
install-operator: 
	...
	sed -e "s,REPLACE_APPR_REPOSITORY,$(POSTGRESQL_APPR_REPOSITORY)," ./$(INSTALL_DIR)/catalogsourceconfig.yaml | oc apply -f -
	sed -e "s,REPLACE_APPR_REPOSITORY,$(POSTGRESQL_APPR_REPOSITORY)," ./$(INSTALL_DIR)/subscription.yaml | oc apply -f -
